...
app.get('/', async (req, res) => {
    try {
        //call API
        const data = await axiosHelper.getCustomObject('2-44141040', 100, 'name,favorite_character,description,hs_object_id', PRIVATE_APP_ACCESS);
...
app.get('/update-cobj/:id?', async (req, res) => {
    try {
        let data = null;
        if(req.params?.id){
            data = await axiosHelper.getCustomObjectById('2-44141040', req.params.id, 'name,favorite_character,description,hs_object_id', PRIVATE_APP_ACCESS);
        }
...
app.post('/update-cobj/:id?', async (req, res) => {
    try {
        //call API
        if(req.params?.id){
            await axiosHelper.updateCustomObject('2-44141040', req.params.id, req.body, PRIVATE_APP_ACCESS);
        }else{
            await axiosHelper.addCustomObject('2-44141040', req.body, PRIVATE_APP_ACCESS);
        }
        res.redirect('/');
...
app.get('/delete-cobj/:id', async (req, res) => {
    try {
        //call API
        if(req.params?.id){
            await axiosHelper.deleteCustomObject('2-44141040', req.params.id, PRIVATE_APP_ACCESS);
        }
        res.redirect('/');
    } catch (error) {
        console.error(error);
